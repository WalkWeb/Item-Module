        ...
        <tr>
            ...
            <td colspan="3">
                <table class="table_content">
                    ...
                    foreach (Legs::getAll() as $drawing) {
                        ...
                        $resist = StatFactory::baseResist($drawing, $material);

                        if ($resist) {
                            $stats .= $translator->trans($resist->getName()) . ' ' . $resist->getValue() . $resist->getSuffix() . '<br />';
                        }

                        $magicDefense = StatFactory::baseMagicDefense($drawing);

                        if ($magicDefense) {
                            $stats .= $translator->trans($magicDefense->getName()) . ' ' . $magicDefense->getValue() . $magicDefense->getSuffix() . '<br />';
                        }

                        $defense = StatFactory::baseDefense($drawing);

                        if ($defense) {
                            $stats .= $translator->trans($defense->getName()) . ' ' . $defense->getValue() . $defense->getSuffix() . '<br />';
                        }

                        foreach ($drawing->getStats() as $stat) {
                            $stats .= $translator->trans($stat->getName()) . ' ' .  $stat->getPrefix() . $stat->getValue() . $stat->getSuffix() . '<br />';
                        }
